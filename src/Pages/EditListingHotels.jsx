...
export default function EditListingHotels() {
  const[geolocationEnabled, setGeolocationEnabled] = useState(false);
    ...
    const { type, name, room, adult, star, address, description, offer, regularPrice, discountedPrice,latitude, longitude,images } = formData;
    
    const [listingHotel , setListingFlights] =useState();

    const params = useParams();
    useEffect(()=>{
        if(listingHotel && listingHotel.userRef !== auth.currentUser.uid){
            toast.error("You cannot edit this listing ")
            navigate("/");
           }
    },[auth.currentUser.uid, listingHotel,navigate])
    useEffect(()=>{
        setLoading(true);
        async function fetchingListingHotel(){
