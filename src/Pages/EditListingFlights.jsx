...
export default function EditListingFlights() {
  const[geolocationEnabled, setGeolocationEnabled] = useState(false);
    ...
    const { type, name, from, dept, to, arr, day, stop, description, offer, regularPrice, discountedPrice,latitude, longitude, images} = formData;
    const [listingFlight , setListingFlights] =useState();

    const params = useParams();
    useEffect(()=>{
        if(listingFlight && listingFlight.userRef !== auth.currentUser.uid){
            toast.error("You cannot edit this listing ")
            navigate("/");
           }
    },[auth.currentUser.uid, listingFlight,navigate])
    useEffect(()=>{
        setLoading(true);
        async function fetchingListingDestination(){
            const docRef  = doc(db,"listing-flight", params.listingID);
            const docSnap = await getDoc(docRef);
            if (docSnap.exists()){
                setListingFlights(docSnap.data());
   ...
   //   let location;
      ...
      //  delete formDataCopy.longitude;
       const docRef = await addDoc(collection(db, "listing-flight"), formDataCopy);
       await updateDoc(docRef, formDataCopy);
       setLoading(false);
       toast.success("Flight Listed");

       
       navigate(`/categoryflight/${formDataCopy.type}/${docRef.id}`)
   }
  ...
  return (
    <main className='max-w-md px-2 mx-auto'><h1 className='text-3xl text-center mt-6 font-bold'>
        Create a listing of Flights
        </h1>
        <form onSubmit={onSubmit}>
            <p className='text-lg mt-6 font-semibold'>Add Flights</p>
            <div className='flex'>
            ...
            <p className='text-lg mt-6 font-semibold'>Name of Airline </p>
            ...
            <div className='flex space-x-6 mt-6 '>
                <div className=''>
                    <p className='w-full text-lg font-semibold'>From: City or Airport Code</p>
                    <input type="text" id="from" value={from} onChange={onChange} required placeholder='From'
                ...
                <div className=''>
                    <p className='w-full text-lg font-semibold'>Departure Time</p>
                    <input type="time" id="dept" value={dept} onChange={onChange} required
                    className='px-4 py-2 text-xl text-grey-700 bg-white border border-grey-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white
                ...
                <div className='flex space-x-6 mt-6 '>
                <div className=''>
                    <p className='w-full text-lg font-semibold'>To: City or Airport Code</p>
                    <input type="text" id="to" value={to} onChange={onChange} required placeholder='To'
                    className='px-4 py-2 text-xl text-grey-700 bg-white border border-grey-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white
                    focus:border-teal-600'/>
                </div>
                <div className=''>
                    <p className='w-full text-lg font-semibold'>Arrival Time</p>
                    <input type="time" id="arr" value={arr} onChange={onChange} required
                    className='px-4 py-2 text-xl text-grey-700 bg-white border border-grey-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white
                    focus:border-teal-600 text-center'/>
                </div>
                </div>
                ...
                <div className=''>
                    <p className='w-full text-lg font-semibold'>No. of days take</p>
                    <input type="number" id="day" value={day} onChange={onChange} min="0" max="5" required
                    className='px-12 py-2 text-xl text-grey-700 bg-white border border-grey-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white
                    focus:border-teal-600 text-center'/>
                </div>
                 <div className=''>
                    <p className='w-full text-lg font-semibold'>No. of stops</p>
                    <input type="number" id="stop" value={stop} onChange={onChange} min="0" max="10" required
                    className='px-12 py-2 text-xl text-grey-700 bg-white border border-grey-300 rounded transition duration-150 ease-in-out focus:text-gray-700 focus:bg-white
                    focus:border-teal-600 text-center'/>
                </div>
                {/*<div className=''>
