...
export default function EditListingDestination() {
    const navigate = useNavigate();
    const auth = getAuth();
    const[geolocationEnabled, setGeolocationEnabled] = useState(false);
    const [loading, setLoading] = useState(false);
    const [listingDestination, setListingDestination] = useState(null);
    ...
    useEffect(()=>{
        if(listingDestination && listingDestination.userRef !== auth.currentUser.uid){
            toast.error("You cannot edit this listing ")
            navigate("/");
           }
    },[auth.currentUser.uid, listingDestination,navigate])
    useEffect(()=>{
        setLoading(true);
        async function fetchingListingDestination(){
            const docRef  = doc(db,"listing-destinations", params.listingID);
            const docSnap = await getDoc(docRef);
            if (docSnap.exists()){
                setListingDestination(docSnap.data());
